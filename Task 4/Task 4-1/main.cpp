...
int main()
{
    ...
    do {
        cout << "Заполнить массив: - 0. С клавиатуры - 1. Случайными числами Введите опцию: ";
        cin >> key;
    } while (key < 0 || key>1);
    fm = FillMethod(key);
    ...
    cin >> c;
    cout << "Сумма четных элементов, которые больше заданного числа: " << SumTask(a, n, c) << endl;

    cout << "Умножаем все четные и положительные числа на последний элемент..." << endl << endl;
    MultLast(a, n);
    PrintArray(a, n);

    int d = PosPair(a, n);
    if (d == -1)
        cout << "Парные элементов с разными знаками не найдено" << endl;
    else
